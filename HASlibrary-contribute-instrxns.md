...
# CONTRIBUTION GUIDELINES — HACK-A-SAT RESOURCE LIBRARY

Defense Digital Service welcomes contributions to the Hack-A-SAT Resource Library for the Space Security Challenge. All contributions are accepted via GitHub Pull Requests.

---

## Before You Submit

- **Search first.** Ensure your link is not already listed or suggested in an open/closed PR.
- **One PR per resource.** Make an individual pull request for each addition.
- **Stay on topic.** Content must relate to satellites, satellite security, ground stations, orbital mechanics, SDR, space systems architecture, or closely related fields.
- **Working links only.** Verify every URL before submitting. Dead links will be rejected.
- **Correct category.** Add your resource under the most relevant existing section.
- **New category?** Add to the most relevant existing section and note `NEW CATEGORY` in your PR description. A maintainer will restructure if appropriate.
